...
serve(async (req) => {
  ...
  try {
    ...
    const adminBotToken = Deno.env.get('TELEGRAM_BOT_TOKEN') || null;
    ...
    // 🔴 إصلاح: استخدام التوكن الموحد للأدمن بدلاً من توكن كل عيادة
    const botToken = adminBotToken;
    if (!botToken) {
      console.error("❌ No TELEGRAM_BOT_TOKEN set in environment");
      return new Response(JSON.stringify({ ok: false, error: "No bot token" }), { status: 500, headers: corsHeaders });
    }
    ...
    for (const a of appts || []) {
      ...
      // 🔴 إصلاح: استخدام التوكن الموحد للأدمن
