...
export default function CashierPage() {
  // 🔥 Error Boundary محلي للكشف عن أي خطأ
  try {
    ...
    const handleIssueInvoice = async () => {
      ...
      try {
        const { error: checkError } = await supabase.from("invoices").select("id").limit(1);
        if (checkError && checkError.message?.includes("relation") && checkError.message?.includes("does not exist")) {
          toast({ title: "⚠️ جدول الفواتير غير موجود", description: "نفذ ملف SQL الخاص بترحيل قاعدة البيانات أولاً.", variant: "destructive" });
          return;
        }
      } catch (e) {
        // تجاهل
    ...
    console.error("🔥 CashierPage Error:", error);
    return (
      <div className="min-h-screen bg-mesh flex items-center justify-center p-4">
        <div className="card-modern p-8 max-w-lg text-center space-y-4">
          <div className="text-3xl text-destructive">⚠️</div>
          <h1 className="text-xl font-bold text-foreground">حدث خطأ غير متوقع</h1>
          <p className="text-sm text-muted-foreground">يرجى التحقق من وحدة التحكم (Console) للحصول على التفاصيل.</p>
          <pre className="bg-muted p-4 rounded-lg text-xs text-left overflow-auto max-h-48">
            {String(error)}
          </pre>
          <Button onClick={() => window.location.reload()} className="w-full">إعادة تحميل الصفحة</Button>
        </div>
      </div>
    );
  }
...
function CashierStats({ appointments, expenses }: { appointments: Appointment[]; expenses: Expense[] }) {
  try {
    ...
    console.error("🔥 CashierStats Error:", error);
    return (
      <div className="p-4 bg-red-50 border border-red-200 rounded-xl text-red-600 text-sm">
        ⚠️ حدث خطأ في عرض الإحصائيات. تحقق من وحدة التحكم.
      </div>
    );
  }
}
